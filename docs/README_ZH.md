...
- 构建缓存
    ...
    我们在容器中进行构建，可以使用Persistent Volumes来保证应用会可以使用上一次的构建缓存。

- 依赖镜像

    完善不同语言的依赖库源，在语言的基础进行镜像使用内部镜像源。

- 问题排查
    
    我们提供构建日志的实时输出和查看，同时可以使用我们的工具登录到构建容器中，进行现场调试。

#### 规范标准

在构建自动化系统的同时，完善标准和规范也是提高团队效率的一个重要手段。

针对持续集成环节，我们要求：
1. Pipeline中必须有测试步骤。
2. 统计测试覆盖率，并报告变化趋势，对于低于预设定阈值的，可以强制停止构建。

#### 高可用集群

我们充分利用Kubernetes的特性来支持构建的工作。
...
在CI工具上，优选原生云组件，例如，Drone来做我们的构建组件。

额外地，我们需要做一些特殊维度的监控，
1. 当前构建任务队列的排队数量
2. 构建任务的最长执行时间，防止出现卡死的构建任务
3. 磁盘、内存、CPU的使用情况

另外，在集群上需要严格的区分构建任务和业务，不能出现构建抢占资源的情况，我们可以有以下方案，
1. 单独的构建集群
2. 使用节点亲和性，在固定节点上跑构建任务
